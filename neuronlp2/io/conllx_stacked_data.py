...
def _generate_stack_inputs(heads, types, cheads, ctypes):
    ...
    cstacked_heads = []
    cstacked_types = []
    cchildren = [0 for _ in range(len(heads)-1)]

    for child in range(len(cheads)):
        if child == 0: continue 
        cstacked_heads.append(child)
        head=cheads[child]
        cchildren[child-1]=head
        cstacked_types.append(ctypes[child])
    ...
    if debug:
        ...
        print 'stacked_types', stacked_types
        print 'children', children
        print 'const stacked_heads', cstacked_heads
        print 'const stacked_types', cstacked_types
        print 'const children', cchildren
        exit(0)
        
    #return stacked_heads, children, siblings, stacked_types, skip_connect, previous, next
    return stacked_heads, children, cstacked_heads, stacked_types, skip_connect, cstacked_types, cchildren


def read_stacked_data(source_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, ctype_alphabet, max_size=None, normalize_digits=True, prior_order='deep_first'):
    data = [[] for _ in _buckets]
    max_char_length = [0 for _ in _buckets]
    print('Reading data from %s' % source_path)
    counter = 0
    reader = CoNLLXReader(source_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, ctype_alphabet)
    ...
    while inst is not None and (not max_size or counter < max_size):
        ...
        for bucket_id, bucket_size in enumerate(_buckets):
            if inst_size < bucket_size:
                stacked_heads, children, siblings, stacked_types, skip_connect, previous, next = _generate_stack_inputs(inst.heads, inst.type_ids, inst.cheads, inst.ctype_ids)
                data[bucket_id].append([sent.word_ids, sent.char_id_seqs, inst.pos_ids, inst.heads, inst.type_ids, inst.cheads, inst.ctype_ids, stacked_heads, children, siblings, stacked_types, skip_connect, previous, next])
...
def read_stacked_data_to_variable(source_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, ctype_alphabet,
                                  max_size=None, normalize_digits=True, prior_order='deep_first', use_gpu=False, volatile=False):
    data, max_char_length = read_stacked_data(source_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, ctype_alphabet, max_size=max_size, normalize_digits=normalize_digits, prior_order=prior_order)
    ...
    for bucket_id in range(len(_buckets)):
        ...
        cohid_inputs = np.empty([bucket_size, bucket_length], dtype=np.int64)
        cotid_inputs = np.empty([bucket_size, bucket_length], dtype=np.int64)
	...
	next_inputs = np.empty([bucket_size, bucket_length - 1], dtype=np.int64)	
        ...
        for i, inst in enumerate(data[bucket_id]):
            wids, cid_seqs, pids, hids, tids, cohids, cotids, stack_hids, chids, ssids, stack_tids, skip_ids, previous_ids, next_ids = inst
            inst_size = len(wids)
            ...
            # const. type ids
            cotid_inputs[i, :inst_size] = cotids
            cotid_inputs[i, inst_size:] = PAD_ID_TAG
            # const. heads
            cohid_inputs[i, :inst_size] = cohids
            cohid_inputs[i, inst_size:] = PAD_ID_TAG
            # masks_e
        ...
        cheads = Variable(torch.from_numpy(cohid_inputs), volatile=volatile)
        ctypes = Variable(torch.from_numpy(cotid_inputs), volatile=volatile)
	...
	next = Variable(torch.from_numpy(next_inputs), volatile=volatile)
        ...
        data_variable.append(((words, chars, pos, heads, types, cheads, ctypes, masks_e, single, lengths_e),
...
def get_batch_stacked_variable(data, batch_size, unk_replace=0.):
    ...
    words, chars, pos, heads, types, cheads, ctypes, masks_e, single, lengths_e = data_encoder
    ...
    return (words, chars[index], pos[index], heads[index], types[index], cheads[index], ctypes[index], masks_e[index], lengths_e[index]), \
...
def iterate_batch_stacked_variable(data, batch_size, unk_replace=0., shuffle=False):
    ...
    for bucket_id in bucket_indices:
        ...
        for start_idx in range(0, bucket_size, batch_size):
            ...
            yield (words[excerpt], chars[excerpt], pos[excerpt], heads[excerpt], types[excerpt], cheads[excerpt], ctypes[excerpt], masks_e[excerpt], lengths_e[excerpt]), \
