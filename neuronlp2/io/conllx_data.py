...
def create_alphabets(alphabet_directory, train_path, data_paths=None, max_vocabulary_size=50000, embedd_dict=None,
    ...
    logger.info("Const. Type Alphabet Size: %d" % ctype_alphabet.size())
    return word_alphabet, char_alphabet, pos_alphabet, type_alphabet, ctype_alphabet


def read_data(source_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, ctype_alphabet, max_size=None,
    ...
    while inst is not None and (not max_size or counter < max_size):
        ...
        for bucket_id, bucket_size in enumerate(_buckets):
            if inst_size < bucket_size:
                data[bucket_id].append([sent.word_ids, sent.char_id_seqs, inst.pos_ids, inst.heads, inst.type_ids, inst.ctype_ids])
...
def get_batch(data, batch_size, word_alphabet=None, unk_replace=0.):
    ...
    ctid_inputs = np.empty([batch_size, bucket_length], dtype=np.int64)
    
    masks = np.zeros([batch_size, bucket_length], dtype=np.float32)
    single = np.zeros([batch_size, bucket_length], dtype=np.int64)

    for b in range(batch_size):
        wids, cid_seqs, pids, hids, tids, chids, ctids = random.choice(data[bucket_id])
        ...
        # const. type ids
        ctid_inputs[b, :inst_size] = ctids
        ctid_inputs[b, inst_size:] = PAD_ID_TAG
        # heads
    ...
    return wid_inputs, cid_inputs, pid_inputs, hid_inputs, tid_inputs, chid_inputs, ctid_inputs, masks
...
def iterate_batch(data, batch_size, word_alphabet=None, unk_replace=0., shuffle=False):
    ...
    for bucket_id in bucket_indices:
        ...
        ctid_inputs = np.empty([bucket_size, bucket_length], dtype=np.int64)
        
        masks = np.zeros([bucket_size, bucket_length], dtype=np.float32)
        single = np.zeros([bucket_size, bucket_length], dtype=np.int64)

        for i, inst in enumerate(data[bucket_id]):
            wids, cid_seqs, pids, hids, tids, ctids = inst
            inst_size = len(wids)
            # word ids
            ...
            # const. type ids
            ctid_inputs[i, :inst_size] = ctids
            ctid_inputs[i, inst_size:] = PAD_ID_TAG
            # heads
        ...
        for start_idx in range(0, bucket_size, batch_size):
            ...
            yield wid_inputs[excerpt], cid_inputs[excerpt], pid_inputs[excerpt], hid_inputs[excerpt], \
                  tid_inputs[excerpt], chid_inputs[excerpt], ctid_inputs[excerpt], masks[excerpt]


def read_data_to_variable(source_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, ctype_alphabet, max_size=None,
                          normalize_digits=True, symbolic_root=False, symbolic_end=False,
                          use_gpu=False, volatile=False):
    data, max_char_length = read_data(source_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, ctype_alphabet,
    ...
    for bucket_id in range(len(_buckets)):
        ...
        for i, inst in enumerate(data[bucket_id]):
            wids, cid_seqs, pids, hids, tids, chids, ctids = inst
            inst_size = len(wids)
            lengths[i] = inst_size
            # word ids
            wid_inputs[i, :inst_size] = wids
        ...
        cheads = Variable(torch.from_numpy(chid_inputs), volatile=volatile)
        ctypes = Variable(torch.from_numpy(ctid_inputs), volatile=volatile)
        ...
        data_variable.append((words, chars, pos, heads, types, cheads, ctypes, masks, single, lengths))
...
def get_batch_variable(data, batch_size, unk_replace=0.):
    ...
    words, chars, pos, heads, types, cheads, ctypes, masks, single, lengths = data_variable[bucket_id]
    bucket_size = bucket_sizes[bucket_id]
    batch_size = min(bucket_size, batch_size)
    ...
    return words, chars[index], pos[index], heads[index], types[index], cheads[index], ctypes[index], masks[index], lengths[index]
    ...
    for bucket_id in bucket_indices:
        ...
        for start_idx in range(0, bucket_size, batch_size):
            if shuffle:
                ...
                excerpt = slice(start_idx, start_idx + batch_size)
            yield words[excerpt], chars[excerpt], pos[excerpt], heads[excerpt], types[excerpt], cheads[excerpt], ctypes[excerpt],\
