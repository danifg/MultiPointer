...
def main():
    ...
    word_alphabet, char_alphabet, pos_alphabet, type_alphabet, ctype_alphabet = conllx_stacked_data.create_alphabets(alphabet_path, train_path, data_paths=[dev_path, test_path],
    ...
    num_ctypes = ctype_alphabet.size()
    
    logger.info("Word Alphabet Size: %d" % num_words)
    logger.info("Character Alphabet Size: %d" % num_chars)
    logger.info("POS Alphabet Size: %d" % num_pos)
    logger.info("Type Alphabet Size: %d" % num_types)
    logger.info("Const. Type Alphabet Size: %d" % num_ctypes)
    
    logger.info("Reading Data")
    use_gpu = torch.cuda.is_available()

    data_train = conllx_stacked_data.read_stacked_data_to_variable(train_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, ctype_alphabet, use_gpu=use_gpu, prior_order=prior_order)
    num_data = sum(data_train[1])

    data_dev = conllx_stacked_data.read_stacked_data_to_variable(dev_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, ctype_alphabet, use_gpu=use_gpu, volatile=True, prior_order=prior_order)
    data_test = conllx_stacked_data.read_stacked_data_to_variable(test_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, ctype_alphabet, use_gpu=use_gpu, volatile=True, prior_order=prior_order)
    ...
    network = NewStackPtrNet(word_dim, num_words, char_dim, num_chars, pos_dim, num_pos, num_filters, window,
                          mode, input_size_decoder, hidden_size, encoder_layers, decoder_layers,
                             num_types, num_ctypes, arc_space, type_space,
                          ...
                          biaffine=True, pos=use_pos, char=use_char, prior_order=prior_order,
                             skipConnect=skipConnect, grandPar=grandPar, sibling=sibling, proj=proj)
    def save_args():
    ...
    cpred_writer = CoNLLXWriter(word_alphabet, char_alphabet, pos_alphabet, ctype_alphabet)
    cgold_writer = CoNLLXWriter(word_alphabet, char_alphabet, pos_alphabet, ctype_alphabet)
    ...
    dev_ucorrect_nopunc = 0.01
    dev_lcorrect_nopunc = 0.01
    ...
    cdev_ucorrect = 0.0
    cdev_lcorrect = 0.0
    cdev_ucomlpete_match = 0.0
    cdev_lcomplete_match = 0.0

    cdev_ucorrect_nopunc = 0.01
    cdev_lcorrect_nopunc = 0.01
    cdev_ucomlpete_match_nopunc = 0.0
    cdev_lcomplete_match_nopunc = 0.0
    cdev_root_correct = 0.0

    cbest_epoch = 0

    ctest_ucorrect = 0.0
    ctest_lcorrect = 0.0
    ctest_ucomlpete_match = 0.0
    ctest_lcomplete_match = 0.0

    ctest_ucorrect_nopunc = 0.0
    ctest_lcorrect_nopunc = 0.0
    ctest_ucomlpete_match_nopunc = 0.0
    ctest_lcomplete_match_nopunc = 0.0
    ctest_root_correct = 0.0
    ctest_total = 0
    ctest_total_nopunc = 0
    ctest_total_inst = 0
    ctest_total_root = 0
	...
	train_total = 0.
        ...
        for batch in range(1, num_batches + 1):
            ...
            word, char, pos, heads, types, cheads, ctypes, masks_e, lengths_e = input_encoder
            #stacked_heads, children, sibling, stacked_types, skip_connect, previous, next, masks_d, lengths_d = input_decoder
	    stacked_heads, children, cstacked_heads, stacked_types, skip_connect, cstacked_types, cchildren, masks_d, lengths_d = input_decoder
	    ...
	    loss_arc, \
            loss_type, \
            loss_cov, num = network.loss(word, char, pos, heads, stacked_heads, children, None, stacked_types, None, None, label_smooth,
	    ...
	    rev_loss_arc, \
            rev_loss_type, \
            rev_loss_cov, rev_num = network.rev_loss(word, char, pos, cheads, cstacked_heads, cchildren, None, cstacked_types, None, None, label_smooth,
                                                            skip_connect=skip_connect, mask_e=masks_e, length_e=lengths_e, mask_d=masks_d, length_d=lengths_d)
	    ...
	    #LOSS DOBLE DECODER
	    #loss = 0.5 * (loss_arc + loss_type + cov * loss_cov) + (rev_loss_arc + rev_loss_type + cov * rev_loss_cov)
            
	    loss = loss_arc + loss_type + cov * loss_cov + (rev_loss_arc + rev_loss_type + cov * rev_loss_cov)
	    loss.backward()
            clip_grad_norm(network.parameters(), clip)
            optim.step()
            train_err_arc += loss_arc.data[0] * num + rev_loss_arc.data[0] * rev_num
            train_err_type += loss_type.data[0] * num + rev_loss_type.data[0]* rev_num
            train_err_cov += loss_cov.data[0] * num + rev_loss_cov.data[0]*rev_num
            train_total += (num + rev_num)	
	...
	start_time_dev = time.time()
        for batch in conllx_stacked_data.iterate_batch_stacked_variable(data_dev, batch_size):
            ...
            word, char, pos, heads, types, cheads, ctypes, masks, lengths = input_encoder
	...
	#===========================================================================================================================
	#						CONSTITUENT DECODING
	#===========================================================================================================================
	print('======EVALUATING PERFORMANCE ON CONSTITUENT DEV======')
	cpred_filename = '%spred_const_dev%d' % (str(uid), epoch)
	cpred_filename = os.path.join(model_path, cpred_filename)
        cpred_writer.start(cpred_filename)
	cgold_filename = '%sgold_const_dev%d' % (str(uid), epoch)
	cgold_filename = os.path.join(model_path, cgold_filename)
        cgold_writer.start(cgold_filename)
        ...
        cdev_ucomlpete = 0.0
        cdev_lcomplete = 0.0
        cdev_ucorr_nopunc = 0.0
        cdev_lcorr_nopunc = 0.0
        cdev_total_nopunc = 0
        cdev_ucomlpete_nopunc = 0.0
        cdev_lcomplete_nopunc = 0.0
        cdev_root_corr = 0.0
        cdev_total_root = 0.0
        cdev_total_inst = 0.0
	cstart_time_dev = time.time()
        for batch in conllx_stacked_data.iterate_batch_stacked_variable(data_dev, batch_size):
            input_encoder, _ = batch
            word, char, pos, heads, types, cheads, ctypes, masks, lengths = input_encoder
            cheads_pred, ctypes_pred, _, _ = network.const_decode(word, char, pos, mask=masks, length=lengths, beam=beam, leading_symbolic=conllx_stacked_data.NUM_SYMBOLIC_TAGS)

            word = word.data.cpu().numpy()
            pos = pos.data.cpu().numpy()
            lengths = lengths.cpu().numpy()
            cheads = cheads.data.cpu().numpy()
            ctypes = ctypes.data.cpu().numpy()

            cpred_writer.write(word, pos, cheads_pred, ctypes_pred, lengths, symbolic_root=True)
            cgold_writer.write(word, pos, cheads, ctypes, lengths, symbolic_root=True)

            stats, stats_nopunc, stats_root, num_inst = parser.eval(word, pos, cheads_pred, ctypes_pred, cheads, ctypes, word_alphabet, pos_alphabet, lengths, punct_set=punct_set, symbolic_root=True)
            ucorr, lcorr, total, ucm, lcm = stats
            ucorr_nopunc, lcorr_nopunc, total_nopunc, ucm_nopunc, lcm_nopunc = stats_nopunc
            corr_root, total_root = stats_root
            ...
            cdev_ucorr_nopunc += ucorr_nopunc
            cdev_lcorr_nopunc += lcorr_nopunc
            cdev_total_nopunc += total_nopunc
            cdev_ucomlpete_nopunc += ucm_nopunc
            cdev_lcomplete_nopunc += lcm_nopunc

            cdev_root_corr += corr_root
            cdev_total_root += total_root

            cdev_total_inst += num_inst

	cend_time_dev = time.time()
	clasted_time_dev=cend_time_dev-cstart_time_dev
        cpred_writer.close()
        cgold_writer.close()
        print('W. Punct: ucorr: %d, lcorr: %d, total: %d, uas: %.2f%%, las: %.2f%%, ucm: %.2f%%, lcm: %.2f%%' % (
            cdev_ucorr, cdev_lcorr, cdev_total, cdev_ucorr * 100 / cdev_total, cdev_lcorr * 100 / cdev_total, cdev_ucomlpete * 100 / cdev_total_inst, cdev_lcomplete * 100 / cdev_total_inst))
        print('Wo Punct: ucorr: %d, lcorr: %d, total: %d, uas: %.2f%%, las: %.2f%%, ucm: %.2f%%, lcm: %.2f%%' % (
            cdev_ucorr_nopunc, cdev_lcorr_nopunc, cdev_total_nopunc, cdev_ucorr_nopunc * 100 / cdev_total_nopunc,
            cdev_lcorr_nopunc * 100 / cdev_total_nopunc, cdev_ucomlpete_nopunc * 100 / cdev_total_inst, cdev_lcomplete_nopunc * 100 / cdev_total_inst))
        print('Root: corr: %d, total: %d, acc: %.2f%%' % (cdev_root_corr, cdev_total_root, cdev_root_corr * 100 / cdev_total_root))


         
        best_LAS_harmonic_mean=2/((1/dev_lcorrect_nopunc)+(1/cdev_lcorrect_nopunc))
        best_UAS_harmonic_mean=2/((1/dev_ucorrect_nopunc)+(1/cdev_ucorrect_nopunc))
        LAS_harmonic_mean=2/((1/dev_lcorr_nopunc)+(1/cdev_lcorr_nopunc))
        UAS_harmonic_mean=2/((1/dev_ucorr_nopunc)+(1/cdev_ucorr_nopunc))
        #if cdev_lcorrect_nopunc < cdev_lcorr_nopunc or (cdev_lcorrect_nopunc == cdev_lcorr_nopunc and cdev_ucorrect_nopunc < cdev_ucorr_nopunc):
        if best_LAS_harmonic_mean<LAS_harmonic_mean or (best_LAS_harmonic_mean==LAS_harmonic_mean and best_UAS_harmonic_mean<UAS_harmonic_mean):
        #if cdev_lcorrect_nopunc < cdev_lcorr_nopunc or (cdev_lcorrect_nopunc == cdev_lcorr_nopunc and cdev_ucorrect_nopunc < cdev_ucorr_nopunc) or dev_lcorrect_nopunc < dev_lcorr_nopunc or (dev_lcorrect_nopunc == dev_lcorr_nopunc and dev_ucorrect_nopunc < dev_ucorr_nopunc):
            ...
            #TEST CONSTITUENTS
            cdev_ucorrect_nopunc = cdev_ucorr_nopunc
            cdev_lcorrect_nopunc = cdev_lcorr_nopunc
            cdev_ucomlpete_match_nopunc = cdev_ucomlpete_nopunc
            cdev_lcomplete_match_nopunc = cdev_lcomplete_nopunc

            cdev_ucorrect = cdev_ucorr
            cdev_lcorrect = cdev_lcorr
            cdev_ucomlpete_match = cdev_ucomlpete
            cdev_lcomplete_match = cdev_lcomplete

            cdev_root_correct = cdev_root_corr

            cbest_epoch = epoch

	    print('======EVALUATING PERFORMANCE ON CONSTITUENT TEST======')
            #pred_filename = 'tmp/%spred_test%d' % (str(uid), epoch)
	    cpred_filename = '%spred_const_test%d' % (str(uid), epoch)
	    cpred_filename = os.path.join(model_path, cpred_filename)
            cpred_writer.start(cpred_filename)
            #gold_filename = 'tmp/%sgold_test%d' % (str(uid), epoch)
	    cgold_filename = '%sgold_const_test%d' % (str(uid), epoch)
	    cgold_filename = os.path.join(model_path, cgold_filename)
            cgold_writer.start(cgold_filename)
	    ...
	    cstart_time_test = time.time()
            for batch in conllx_stacked_data.iterate_batch_stacked_variable(data_test, batch_size):
                ...
                ctest_ucorrect += ucorr
                ctest_lcorrect += lcorr
                ctest_total += total
                ctest_ucomlpete_match += ucm
                ctest_lcomplete_match += lcm

                ctest_ucorrect_nopunc += ucorr_nopunc
                ctest_lcorrect_nopunc += lcorr_nopunc
                ctest_total_nopunc += total_nopunc
                ctest_ucomlpete_match_nopunc += ucm_nopunc
                ctest_lcomplete_match_nopunc += lcm_nopunc

                ctest_root_correct += corr_root
                ctest_total_root += total_root

                ctest_total_inst += num_inst

	    cend_time_test = time.time()
	    clasted_time_test=cend_time_test-cstart_time_test
            cpred_writer.close()
            cgold_writer.close()

        else:
            currentUASdep = dev_ucorr_nopunc * 100 / dev_total_nopunc
            bestUASdep = dev_ucorrect_nopunc * 100 / dev_total_nopunc
            currentUAScon = cdev_ucorr_nopunc * 100 / cdev_total_nopunc
            bestUAScon = cdev_ucorrect_nopunc * 100 / cdev_total_nopunc
                        
            best_UAS_harmonic_mean=2/((1/bestUASdep)+(1/bestUAScon))
            current_UAS_harmonic_mean=2/((1/currentUASdep)+(1/currentUAScon)) 
            #if cdev_ucorr_nopunc * 100 / cdev_total_nopunc < cdev_ucorrect_nopunc * 100 / cdev_total_nopunc - 5 or patient >= schedule:
            if current_UAS_harmonic_mean < best_UAS_harmonic_mean - 5 or patient >= schedule:
                # network = torch.load(model_name)
                network.load_state_dict(torch.load(model_name))
                lr = lr * decay_rate
                optim = generate_optimizer(opt, lr, network.parameters())
        ...
        print('CONSTITUENT')        
        print('----------------------------------------------------------------------------------------------------------------------------')
	print('TIME DEV: ', clasted_time_dev, 'NUM SENTS DEV: ', cdev_total_root, 'SPEED DEV: ', cdev_total_root/clasted_time_dev)
	print('best dev  W. Punct: ucorr: %d, lcorr: %d, total: %d, uas: %.2f%%, las: %.2f%%, ucm: %.2f%%, lcm: %.2f%% (epoch: %d)' % (
            cdev_ucorrect, cdev_lcorrect, cdev_total, cdev_ucorrect * 100 / cdev_total, cdev_lcorrect * 100 / cdev_total,
            cdev_ucomlpete_match * 100 / cdev_total_inst, cdev_lcomplete_match * 100 / cdev_total_inst,
            cbest_epoch))
        print('best dev  Wo Punct: ucorr: %d, lcorr: %d, total: %d, uas: %.2f%%, las: %.2f%%, ucm: %.2f%%, lcm: %.2f%% (epoch: %d)' % (
            cdev_ucorrect_nopunc, cdev_lcorrect_nopunc, cdev_total_nopunc,
            cdev_ucorrect_nopunc * 100 / cdev_total_nopunc, cdev_lcorrect_nopunc * 100 / cdev_total_nopunc,
            cdev_ucomlpete_match_nopunc * 100 / cdev_total_inst, cdev_lcomplete_match_nopunc * 100 / cdev_total_inst,
            cbest_epoch))
        print('best dev  Root: corr: %d, total: %d, acc: %.2f%% (epoch: %d)' % (cdev_root_correct, cdev_total_root, cdev_root_correct * 100 / cdev_total_root, cbest_epoch))
        print('----------------------------------------------------------------------------------------------------------------------------')
	print('TIME TEST: ', clasted_time_test, 'NUM SENTS TEST: ', ctest_total_root, 'SPEED TEST: ', ctest_total_root/clasted_time_test)
        print('best test W. Punct: ucorr: %d, lcorr: %d, total: %d, uas: %.2f%%, las: %.2f%%, ucm: %.2f%%, lcm: %.2f%% (epoch: %d)' % (
            ctest_ucorrect, ctest_lcorrect, ctest_total, ctest_ucorrect * 100 / ctest_total, ctest_lcorrect * 100 / ctest_total,
            ctest_ucomlpete_match * 100 / ctest_total_inst, ctest_lcomplete_match * 100 / ctest_total_inst,
            cbest_epoch))
        print('best test Wo Punct: ucorr: %d, lcorr: %d, total: %d, uas: %.2f%%, las: %.2f%%, ucm: %.2f%%, lcm: %.2f%% (epoch: %d)' % (
            ctest_ucorrect_nopunc, ctest_lcorrect_nopunc, ctest_total_nopunc,
            ctest_ucorrect_nopunc * 100 / ctest_total_nopunc, ctest_lcorrect_nopunc * 100 / ctest_total_nopunc,
            ctest_ucomlpete_match_nopunc * 100 / ctest_total_inst, ctest_lcomplete_match_nopunc * 100 / ctest_total_inst,
            cbest_epoch))
        print('best test Root: corr: %d, total: %d, acc: %.2f%% (epoch: %d)' % (ctest_root_correct, ctest_total_root, ctest_root_correct * 100 / ctest_total_root, cbest_epoch))
        print('============================================================================================================================')
